 ...
 */
class Model_Api_Signup extends Model_RestAPI {
	...
	public function create($params)
	{
		...
		$json = array();
		$success = false;
		if(!empty($data) && isset($data['email']) && isset($data['password']) && isset($data['fullname'])) {
			$customermodel = App::model('user');
			try {
				$customermodel->setData($data);
				$validate = $customermodel->validateFrontEnd();				
		...
		if($success) {
			...
			$url1 ='<a href="'.$siteUrl.'signup/confirmation?key='.$customer->getVerificationKey().'&email='.$customer->getPrimaryEmailAddress().'&u_password='.$data['password'].'"> This Confirmation Link </a>';
			$content = array("customer" => $customer,"first_name" => $data['fullname'], "confirmation_link" => $url1);		
			$email=App::model('core/email')->smtp($from,$from_name,$customer->getPrimaryEmailAddress(),$subject,$content,$template); 
			$json['success'] = __('User Information Added');		
		}		
			...
			if(!empty($user_profile)) {
				...
				if(!$exisit_user->getUserId()){
					...
					if(!empty($user_profile) && isset($user_profile['email'])) {						
						try {						
							$customermodel = App::model('user')->load($user_profile['email'],'primary_email_address');						
							$customer = $customermodel->getData();								
							if(!$customer) {
								throw new Exception(__('Invalid credentials'));
							}												
							$json['userinfo'] = $customer;		
							$json['success'] = true;					
						} catch(Exception $e) {				
							$json['error'] = $e->getMessage();
						}
